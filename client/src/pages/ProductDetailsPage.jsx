...
function getProductImages(product) {
  ...
  return images
    .map((image) => getImageSource(image, { width: 420 }))
    .filter(Boolean);
}
  ...
  useEffect(() => {
    ...
    async function loadProduct() {
      try {
        ...
        if (
          selectedProduct?.id &&
          !String(selectedProduct.id).startsWith("fallback-")
        ) {
          const reviewsResponse = await publicRequest(
            `/products/${selectedProduct.id}/reviews`,
          );
        ...
        <>
          <section className="page-shell product-detail">
            <div className="product-detail__gallery">
              <div className="product-detail__main">
                {mainImageUrl ? (
                  ...
                  <div
                    className="product-detail__empty-image"
                  ...
                  productImages.map((image, index) => {
                    return (
                      <button
                        ...
                        onClick={() =>
                          setSelectedImage(
                            getImageSource(image, { width: 1200 }) || "",
                          )
                        }
                      >
                        <img
                          src={getImageSource(image, { width: 320 })}
            ...
            <div className="product-detail__content">
              ...
              <h2>{product.name}</h2>
              <p className="rating">
                {reviews.length} Reviews | Add Your Reviews
              </p>
              ...
              {addedMessage ? (
                <p
                  className="text-sm"
                  style={{ color: "#63ac18", marginTop: 8, fontWeight: 600 }}
                >
                  {addedMessage}
                </p>
              ) : null}
              {wishlistMessage ? (
                <p
                  className="text-sm"
                  style={{ color: "#0f766e", marginTop: 8, fontWeight: 600 }}
                >
                  {wishlistMessage}
                </p>
              ) : null}
            ...
            <div className="tabs-block__content">
              {tab === "Description" ? (
                <div className="product-description">
                  ...
                  <p>
                    {product.ingredients ||
                      "Key ingredients are selected for traditional relevance and investigated for modern therapeutic potential. Greenrut emphasizes potency, standardization, and safe use across its formulations."}
                  </p>
                  <h3>The Greenrut Promise: Proven Efficacy, Zero Toxicity</h3>
                  <p>
                    {product.scientificValidation ||
                      "Every product has room for clear scientific validation notes, potency summaries, and toxicology assurance from the admin panel."}
                  </p>
                  <h3>Simple Steps for Optimal Results</h3>
                  <p>
                    {product.directions ||
                      "Follow product directions carefully, use consistently as recommended, and consult a healthcare professional if you are pregnant, nursing, or currently taking medication."}
                  </p>
                  <h3>Important Information</h3>
                  {product.warnings ? <p>{product.warnings}</p> : null}
                  <ul>
                    <li>
                      Review all usage instructions before taking any herbal
                      product.
                    </li>
                    <li>
                      Keep products sealed, dry, and away from direct sunlight.
                    </li>
                    <li>
                      Greenrut prioritizes safety, transparency, and
                      evidence-led claims.
                    </li>
                  </ul>
                </div>
                ...
                <div className="product-reviews">
                  {reviews.length ? (
                    ...
                    <p>
                      No reviews yet. Be the first to leave a review for this
                      product.
                    </p>
                  )}
                  <form
                  ...
                  >
                    ...
                    <select
                      ...
                      onChange={updateReviewField("rating")}
                    >
          ...
          <div className="product-grid product-grid--related !grid !grid-cols-1 xs:!grid-cols-2 xl:!grid-cols-4 !gap-[18px]">
            {relatedProducts.map((item, index) => (
              <button
                ...
                type="button"
                className="text-left"
                onClick={() => onNavigate?.(`/product-details?id=${item.id}`)}
              >
