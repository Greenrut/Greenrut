...
export function LibraryResourcePage({ onNavigate }) {
  ...
  useEffect(() => {
    ...
    async function loadResource() {
      try {
        ...
        if (!cancelled)
          setError(requestError.message || "Failed to load resource");
      } finally {
  ...
  const libraryItems = resources.length
    ? resources
    : fallbackResources.map(normalizeResource);
  const resource = useMemo(() => {
    if (!resourceId) return null;
    return (
      libraryItems.find((item) => String(item.id) === String(resourceId)) ||
      null
    );
  ...
  if (error && !resource) {
    return (
      <section className="page-shell py-12">
        ...
        <button
          type="button"
          className="mt-4 border border-[#d8d6d0] px-4 py-3 text-[11px] font-semibold uppercase"
  ...
  if (!resource) {
    return (
      <section className="page-shell py-12">
        <h1 className="font-serif text-[28px] text-[#2e2a26]">
          Resource not found
        </h1>
        <p className="mt-3 text-sm text-[#6f6b64]">
          The herb or library resource you opened is not available.
        </p>
        <button
          type="button"
          className="mt-4 border border-[#d8d6d0] px-4 py-3 text-[11px] font-semibold uppercase"
  ...
  return (
    <section className="page-shell py-6 xs:py-8 lg:py-10">
      ...
      >
        <div className="max-w-[620px] text-white">
          <p className="text-[10px] uppercase tracking-[0.24em] text-white/70">
            {resource.type || resource.section}
          </p>
          <h1 className="mt-2 font-serif text-[30px] leading-tight xs:text-[36px]">
            {resource.title}
          </h1>
          {resource.localName ? (
            <p className="mt-2 text-[13px] text-white/80">
              Local name: {resource.localName}
            </p>
          ) : null}
        </div>
      </div>
      ...
      <button
        type="button"
        className="mb-5 text-[11px] font-semibold uppercase text-[#2e2a26] underline"
        onClick={() => onNavigate?.("/library")}
      >
        Back to library
      </button>
      ...
      <article className="grid gap-6 border border-[#efefef] bg-white p-5 sm:grid-cols-[280px_minmax(0,1fr)] lg:p-7 reveal-on-scroll reveal-slide-up">
        <div className="bg-[#f5f5f1]">
          {resource.image ? (
            ...
            <div className="grid min-h-[240px] place-items-center text-[12px] text-[#9a958c]">
              No image
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <h2 className="mt-2 font-serif text-[26px] text-[#2e2a26]">
            {resource.title}
          </h2>
          <p className="mt-4 text-[14px] leading-7 text-[#4a453f]">
            {resource.excerpt ||
              "No summary has been published for this resource yet."}
          </p>

          <dl className="mt-6 grid gap-4 text-[13px] leading-6 text-[#5f5a54]">
            {resource.therapeuticUse ? (
              <div>
                <dt className="font-semibold text-[#2e2a26]">
                  Therapeutic use
                </dt>
                <dd>{resource.therapeuticUse}</dd>
              </div>
            ) : null}
            {resource.preparationMethod ? (
              <div>
                <dt className="font-semibold text-[#2e2a26]">
                  Preparation method
                </dt>
                <dd>{resource.preparationMethod}</dd>
              </div>
            ) : null}
            {resource.dosage ? (
              <div>
                <dt className="font-semibold text-[#2e2a26]">Dosage</dt>
                <dd>{resource.dosage}</dd>
              </div>
            ) : null}
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            {resource.resourceUrl ? (
              <button
                type="button"
                className="bg-[#63ac18] px-4 py-3 text-[11px] font-semibold uppercase text-white"
                onClick={() =>
                  window.open(
                    resource.resourceUrl,
                    "_blank",
            ...
            {resource.linkedProductId ? (
              <button
                type="button"
                className="border border-[#d8d6d0] px-4 py-3 text-[11px] font-semibold uppercase text-[#2e2a26]"
                onClick={() =>
                  onNavigate?.(
                    `/product-details?id=${resource.linkedProductId}`,
                  )
                }
              >
                View linked product
